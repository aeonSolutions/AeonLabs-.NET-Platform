...
namespace XComponent.SliderBar
{
    ...
    internal class ColorHelper
    {
        ...
        private static int SoftLightMath(int ibase, int blend)
        {
            float dbase;
            float dblend;
            dbase = ibase / (float)255;
            dblend = blend / (float)255;
            if (dblend < 0.5)
            {
                return Conversions.ToInteger(Conversion.Fix((2 * dbase * dblend + Math.Pow(dbase, 2) * (1 - 2 * dblend)) * 255));
            }
            else
            {
                return Conversions.ToInteger(Conversion.Fix((Math.Sqrt(dbase) * (2 * dblend - 1) + 2 * dbase * (1 - dblend)) * 255));
            }
        }
        ...
        public static int OverlayMath(int ibase, int blend)
        {
            double dbase;
            double dblend;
            dbase = ibase / (double)255;
            dblend = blend / (double)255;
            if (dbase < 0.5)
            {
                return Conversions.ToInteger(Conversion.Fix(2 * dbase * dblend * 255));
            }
            else
            {
                return Conversions.ToInteger(Conversion.Fix((1 - 2 * (1 - dbase) * (1 - dblend)) * 255));
            }
        }
    }
}